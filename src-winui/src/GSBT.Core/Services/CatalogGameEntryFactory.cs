...
public static class CatalogGameEntryFactory
{
    ...
    {
        catalogManager.NormalizeDetectedDisplayNames();
        ...
        for (var i = 0; i < names.Count; i++)
        {
            ...
            var entry = FromCatalogRow(catalogManager, name, row, backupRoot, subfolderPerGame, dateFormat);
            if (entry is null)
            {
                continue;
            }
        ...
        string? dateFormat = null)
    {
        ...
        var lastBkDisplay = string.IsNullOrWhiteSpace(lastBkRaw)
            ? "Not yet"
            : BackupDateFormatter.FormatDisplay(lastBkRaw, dateFormat);
