...
public static class CliHelpContent
{
    public static void WriteMainMenu()
    {
        ...
        AnsiConsole.WriteLine("  gsbt verify            Verify retained backup snapshots");
        AnsiConsole.WriteLine("  gsbt restore           Restore one verified snapshot");
        AnsiConsole.WriteLine("  gsbt settings          View or change settings");
        AnsiConsole.WriteLine("  gsbt add custom        Add a custom folder backup");
        AnsiConsole.WriteLine("  gsbt status            Paths and settings summary");
        if (CliInstallationState.IsGuiInstalled)
        {
            AnsiConsole.WriteLine("  gsbt gui               Open the WinUI desktop app");
        }
        else
        {
            AnsiConsole.WriteLine("  gsbt get gui           Install the WinUI desktop app");
        }
        AnsiConsole.WriteLine("  gsbt diagnostics       Export a redacted operation report");
    ...
    public static void WriteHub()
    {
        ...
        WriteCommandSummary("verify", "Verify retained backup files and checkpoints");
        WriteCommandSummary("restore", "Restore one verified snapshot with rollback protection");
        WriteCommandSummary("settings", "Backup path, compression, retention");
        WriteCommandSummary("add", "Add a custom folder backup entry");
        ...
        {
            WriteCommandSummary("get", "Install the WinUI desktop app");
        }
        WriteCommandSummary("diagnostics", "Export a redacted local operation report");
        AnsiConsole.WriteLine();
        ...
        AnsiConsole.WriteLine("  --ai   JSON result + progress events on stderr + no interactive prompts");
        AnsiConsole.WriteLine("         Use on status, scan, list, backup, compress, verify, restore,");
        AnsiConsole.WriteLine("         settings and diagnostics (implies --json)");
        AnsiConsole.WriteLine("         Example: gsbt backup --ai");
        AnsiConsole.WriteLine();
        AnsiConsole.MarkupLine("[bold]Cancel[/]");
        AnsiConsole.WriteLine("  Press Esc to cancel an interactive prompt before work begins.");
    ...
    public static bool TryWriteCommandHelp(string? command)
    {
        ...
        switch (command.Trim().ToLowerInvariant())
        {
            ...
            case "status":
                WriteStatusHelp();
                return true;
            case "gui" when CliInstallationState.IsGuiInstalled:
                WriteGuiHelp();
                return true;
            case "get" when !CliInstallationState.IsGuiInstalled:
                WriteGetHelp();
                return true;
            case "diagnostics":
                WriteCommandHeader("diagnostics", "Export redacted local operation history and environment details.");
                AnsiConsole.WriteLine("  gsbt diagnostics");
                AnsiConsole.WriteLine("  gsbt diagnostics --output <file>");
                AnsiConsole.WriteLine("  gsbt diagnostics --ai");
                return true;
            default:
                return false;
        }
    ...
    private static void WriteScanHelp()
    {
        ...
        AnsiConsole.WriteLine("  gsbt scan --full");
        ...
        AnsiConsole.WriteLine("  --full               Recheck titles previously remembered as not found");
    ...
    private static void WriteBackupHelp()
    {
        ...
        AnsiConsole.WriteLine("  --ai             JSON result + progress events on stderr + no prompts");
    ...
    private static void WriteCompressHelp()
    {
        ...
        AnsiConsole.WriteLine("Options:");
        AnsiConsole.WriteLine("  --json   JSON result on stdout");
        AnsiConsole.WriteLine("  --ai     JSON result + progress events on stderr");
        AnsiConsole.WriteLine();
    ...
    {
        WriteCommandHeader("add custom", "Register any existing folder as a custom backup entry.");
        AnsiConsole.WriteLine();
        AnsiConsole.WriteLine("Usage:");
        AnsiConsole.WriteLine("  gsbt add custom \"Game Name\" \"C:\\path\\to\\saves\"");
        AnsiConsole.WriteLine("  gsbt add custom \"Warcraft III Custom Maps\" \"C:\\verified\\maps\" --ai");
        AnsiConsole.WriteLine();
        AnsiConsole.WriteLine("The folder must exist. It can contain saves, maps, mods, profiles,");
        AnsiConsole.WriteLine("projects, or other user-selected data. Run gsbt list found afterward.");
        AnsiConsole.WriteLine();
        AnsiConsole.WriteLine("Options:");
        AnsiConsole.WriteLine("  --json   Machine-readable JSON result");
        AnsiConsole.WriteLine("  --ai     Agent-safe JSON result with no interactive prompts");
    }
    ...
    private static void WriteVerifyHelp()
    {
        WriteCommandHeader("verify", "Verify the newest retained snapshot for selected games.");
        AnsiConsole.WriteLine();
        AnsiConsole.WriteLine("Usage:");
        AnsiConsole.WriteLine("  gsbt verify [targets...]");
        AnsiConsole.WriteLine("  gsbt verify 1,3,5 --full");
        AnsiConsole.WriteLine("  gsbt verify --ai");
        AnsiConsole.WriteLine();
        AnsiConsole.WriteLine("Options:");
        AnsiConsole.WriteLine("  --full   Compare SHA-256 content hashes (slower)");
        AnsiConsole.WriteLine("  --json   Machine-readable JSON on stdout");
        AnsiConsole.WriteLine("  --ai     JSON result + progress events on stderr");
    }

    private static void WriteRestoreHelp()
    {
        WriteCommandHeader("restore", "Restore one verified retained snapshot.");
        AnsiConsole.WriteLine();
        AnsiConsole.WriteLine("Usage:");
        AnsiConsole.WriteLine("  gsbt restore <game> --dry-run");
        AnsiConsole.WriteLine("  gsbt restore <game> --snapshot latest");
        AnsiConsole.WriteLine("  gsbt restore <game> --snapshot <run-id> --yes");
        AnsiConsole.WriteLine("  gsbt restore <game> --to <folder> --yes");
        AnsiConsole.WriteLine("  gsbt restore <game> --ai --dry-run");
        AnsiConsole.WriteLine();
        AnsiConsole.WriteLine("Options:");
        AnsiConsole.WriteLine("  --snapshot <id>   latest, run ID, timestamp text, or retained path");
        AnsiConsole.WriteLine("  --mode <mode>     replace (default) or merge");
        AnsiConsole.WriteLine("  --to <folder>     Restore to another folder");
        AnsiConsole.WriteLine("  --dry-run         Preview and fully verify without changing data");
        AnsiConsole.WriteLine("  --yes             Confirm without a prompt (required with --ai)");
        AnsiConsole.WriteLine("  --ai              JSON result + progress events on stderr");
        AnsiConsole.WriteLine();
        AnsiConsole.WriteLine("Restore-specific exits: 3 unsafe target, 4 insufficient space,");
        AnsiConsole.WriteLine("  5 verification failed, 6 rolled back, 7 manual recovery, 8 partial.");
    }

    private static void WriteGetHelp()
    {
        WriteCommandHeader("get gui", "Download and install the WinUI desktop app beside the CLI.");
        AnsiConsole.WriteLine();
        AnsiConsole.WriteLine("Usage:");
        ...
        AnsiConsole.WriteLine();
        AnsiConsole.WriteLine("Options:");
        AnsiConsole.WriteLine("  --installer-url <url>   Use a specific GSBT_Setup_*.exe URL");
        AnsiConsole.WriteLine("  --force                 Reinstall even when already current");
        AnsiConsole.WriteLine("  --allow-custom-host     Trust an explicit non-GitHub HTTPS host");
        AnsiConsole.WriteLine("  --ai                    JSON result + progress events on stderr");
    }
