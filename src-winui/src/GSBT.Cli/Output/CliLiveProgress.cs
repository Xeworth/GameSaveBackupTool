...
public sealed record CliBranchEntry(string GameName, CliBranchStatus Status, string Message = "");
...
public sealed class CliLiveProgress : IDisposable
{
    ...
    public static string WarningText(string text) => $"\x1b[33m{text}\x1b[0m";

    public static string ErrorText(string text) => $"\x1b[31m{text}\x1b[0m";
    ...
    {
        var gameName = entry.Status switch
        {
            CliBranchStatus.Warning => WarningText(entry.GameName),
            CliBranchStatus.Error => ErrorText(entry.GameName),
            _ => entry.GameName,
        };
        Console.Error.WriteLine($"{FormatBranchPrefix(index)}{gameName}");
    }
