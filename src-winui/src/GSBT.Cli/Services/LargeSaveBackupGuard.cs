...
public static class LargeSaveBackupGuard
{
        ...
        bool nonInteractive)
    {
        ...
        foreach (var entry in entries)
        {
            ...
            var confirmation = ConfirmInteractive(entry, bytes, severity);
            if (confirmation == CliConfirmation.Canceled)
            {
                return new FilterResult([], skipped, Canceled: true);
            }

            if (confirmation == CliConfirmation.Declined)
            {
                skipped.Add(new LargeSaveSkip(
                    entry.GameName,
                    bytes,
    ...
    private static CliConfirmation ConfirmInteractive(CatalogGameEntry entry, long bytes, BackupSizeSeverity severity)
    {
        ...
        return CliPrompt.Confirm("  Back up this folder anyway?", defaultValue: false);
    }
