...
from core.backup_paths import ensure_backup_directory, suggested_default_backup_path
from config.app_config import DEFAULT_UI_THEME, app_settings, normalize_ui_theme
from config.settings_defaults import settings_bool, settings_str
from core.window_size import PRESET_800, PRESET_1024, PRESET_CUSTOM, classify_client_size, normalize_preset
...
class AddCustomGameDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        name_layout = QVBoxLayout(self._name_panel)
        ...
        self.layout.addWidget(self._name_panel, 0)

        self._folder_panel = QWidget()
        folder_panel_layout = QVBoxLayout(self._folder_panel)
        folder_panel_layout.setContentsMargins(0, 0, 0, 0)
        folder_panel_layout.setSpacing(6)
        self.folder_label = QLabel("Save Game Folder Location:")
        folder_panel_layout.addWidget(self.folder_label)
        ...
        folder_panel_layout.addLayout(path_layout)

        self._registry_panel = QWidget()
        registry_panel_layout = QVBoxLayout(self._registry_panel)
        registry_panel_layout.setContentsMargins(0, 0, 0, 0)
        registry_panel_layout.setSpacing(6)
        self.registry_label = QLabel("Registry key path (from Regedit address bar):")
        registry_panel_layout.addWidget(self.registry_label)
        self.registry_path_input = QLineEdit()
        self.registry_path_input.setPlaceholderText(r"HKCU\Software\…")
        registry_panel_layout.addWidget(self.registry_path_input)

        self._mode_stack = QStackedWidget()
        self._mode_stack.setSizePolicy(QSizePolicy.Policy.Preferred, QSizePolicy.Policy.Fixed)
        self._mode_stack.addWidget(self._folder_panel)
        self._mode_stack.addWidget(self._registry_panel)
        self.layout.addWidget(self._mode_stack, 0)
        ...
        self.radio_folder = QRadioButton("Save folder on disk")
        self.radio_registry = QRadioButton("Registry / in-key save")
        self.radio_folder.setChecked(True)
        self._bg.addButton(self.radio_folder)
        self._bg.addButton(self.radio_registry)
        mode_row.addWidget(self.radio_folder)
        mode_row.addWidget(self.radio_registry)
        mode_row.addStretch(1)
        self.layout.addLayout(mode_row)

        self.radio_folder.toggled.connect(self._on_mode_changed)
        self.radio_registry.toggled.connect(self._on_mode_changed)

        self.layout.addSpacing(8)
        button_box = QDialogButtonBox(QDialogButtonBox.StandardButton.Ok | QDialogButtonBox.StandardButton.Cancel)
        button_box.accepted.connect(self.accept)
        button_box.rejected.connect(self.reject)
        self.layout.addWidget(button_box, 0)
        self._sync_mode_stack_height()

    def _sync_mode_stack_height(self) -> None:
        page = self._mode_stack.currentWidget()
        if not page:
            return
        page.ensurePolished()
        h = page.sizeHint().height()
        self._mode_stack.setMinimumHeight(h)
        self._mode_stack.setMaximumHeight(h)

    def _on_mode_changed(self):
        reg = self.radio_registry.isChecked()
        self._mode = self.MODE_REGISTRY if reg else self.MODE_FOLDER
        self._mode_stack.setCurrentIndex(1 if reg else 0)
        self._sync_mode_stack_height()
...
class AssignRegistrySaveDialog(QDialog):
    """Prompt for a registry key when assigning save location to an existing row."""

    def __init__(self, game_name: str, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Add registry path")
        self.setMinimumWidth(420)
        StyleManager.instance().set_theme(
            normalize_ui_theme(app_settings().value("ui_theme", DEFAULT_UI_THEME, type=str))
        )
        StyleManager.instance().refresh()
        self.setStyleSheet(StyleManager.instance().small_dialog_qss())
        lay = QVBoxLayout(self)
        lay.addWidget(QLabel(f"Registry save location for <b>{game_name}</b>"))
        lay.addWidget(QLabel("Paste the path from Registry Editor’s address bar, e.g. HKCU\\Software\\…"))
        self.registry_input = QLineEdit()
        self.registry_input.setPlaceholderText(r"HKCU\Software\…")
        lay.addWidget(self.registry_input)
    ...
    def get_registry_path(self) -> str:
        return self.registry_input.text().strip()
...
class FirstBackupDestinationDialog(QDialog):
    ...
    def __init__(self, parent=None, initial_path=""):
        ...
        prefill = (initial_path or "").strip() or suggested_default_backup_path()
        self.path_input.setText(prefill)
    ...
    def _browse(self):
        current = (
            ...
            or self.settings.value("default_backup_path", "", type=str)
            or suggested_default_backup_path()
        )
    ...
    def _on_ok(self):
        ...
        ok, err = ensure_backup_directory(folder)
        if not ok:
            QMessageBox.warning(self, "Invalid Folder", err or "Please choose a valid folder.")
            return
        self.selected_folder = os.path.normpath(os.path.abspath(os.path.expanduser(folder)))
        self.remember_default = self.dont_show_checkbox.isChecked()
        if self.remember_default:
            self.settings.setValue("default_backup_path", self.selected_folder)
            self.settings.setValue("last_backup_path", self.selected_folder)
        self.accept()
...
class SettingsDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self._suppress_window_live_preview = False
        ...
        self.ask_compress_on_exit_checkbox.setChecked(settings_bool(self.settings, "ask_compress_on_exit"))
        ...
        themes_form.addRow("Theme:", self.theme_combo)
        themes_form.addRow("Language:", self.lang_combo)
        ...
        self.notifications_checkbox.setChecked(settings_bool(self.settings, "notifications_enabled"))
        ...
        self.notification_sound_checkbox.setChecked(settings_bool(self.settings, "notification_sound_enabled"))
        ...
        self.minimize_to_tray_checkbox.setChecked(settings_bool(self.settings, "minimize_to_tray"))
        system_form.addRow(self.minimize_to_tray_checkbox)

        self.main_window_size_combo = QComboBox()
        self.main_window_size_combo.addItem("800 × 600 (nominal)", PRESET_800)
        self.main_window_size_combo.addItem("1024 × 768 (nominal)", PRESET_1024)
        self.main_window_size_combo.addItem("Custom (last window size)", PRESET_CUSTOM)
        self.main_window_size_combo.setMaximumWidth(227)
        self._suppress_window_live_preview = True
        win_preset = normalize_preset(settings_str(self.settings, "main_window_client_preset"))
        win_idx = self.main_window_size_combo.findData(win_preset)
        if win_idx >= 0:
            self.main_window_size_combo.setCurrentIndex(win_idx)
        self._suppress_window_live_preview = False
        self.main_window_size_combo.currentIndexChanged.connect(self._try_main_window_live_preview)
        system_form.addRow("Main window size:", self.main_window_size_combo)

        self.lock_resolution_checkbox = CustomCheckBox("Lock this resolution")
        self.lock_resolution_checkbox.setChecked(settings_bool(self.settings, "main_window_lock_resolution"))
        self.lock_resolution_checkbox.toggled.connect(self._try_main_window_live_preview)
        system_form.addRow(self.lock_resolution_checkbox)
    ...
    def _try_main_window_live_preview(self) -> None:
        if self._suppress_window_live_preview:
            return
        parent = self.parent()
        if parent is None or not hasattr(parent, "apply_window_layout_preview"):
            return
        preset = normalize_preset(self.main_window_size_combo.currentData())
        ...
        if preset == PRESET_CUSTOM:
            custom_w, custom_h = parent.width(), parent.height()
        parent.apply_window_layout_preview(
            preset,
            custom_w,
            custom_h,
            self.lock_resolution_checkbox.isChecked(),
        )
    ...
    def browse_backup_folder(self):
        ...
        current_path = self.backup_folder_input.text() or suggested_default_backup_path()
    ...
    def accept(self):
        ...
        lock_resolution = self.lock_resolution_checkbox.isChecked()
        self.settings.setValue("main_window_lock_resolution", lock_resolution)
        parent = self.parent()
        if lock_resolution and parent is not None and hasattr(parent, "width"):
            w, h = parent.width(), parent.height()
            classified = classify_client_size(w, h)
            self.settings.setValue("main_window_client_preset", classified)
            self.settings.setValue("main_window_custom_width", w)
            self.settings.setValue("main_window_custom_height", h)
        else:
            preset = normalize_preset(self.main_window_size_combo.currentData())
            self.settings.setValue("main_window_client_preset", preset)
            if preset == PRESET_CUSTOM and parent is not None and hasattr(parent, "width"):
                self.settings.setValue("main_window_custom_width", parent.width())
                self.settings.setValue("main_window_custom_height", parent.height())
