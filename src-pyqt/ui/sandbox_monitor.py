...
class SandboxMonitorWindow(QMainWindow):
    ...
    _TRACE_INTRO_LINES = (
        "Legend (per-game END line): SAVE_ON_DISK | MANIFEST_PATHS_NO_DISK (manifest had paths, not on disk) | "
        "NO_MANIFEST_PATHS | REGISTRY_IN_KEY",
        "Lookup is offline via bundled Ludusavi manifest (name_index + steam_index); optional GitHub refresh from scan menu.",
    )
    ...
    def __init__(self, parent=None):
        ...
        self._push_batch_lines(
            "Save fetch (batches): each scan’s manifest lookup logs a header when fetch starts and a footer when all games finish.\n"
        )
    ...
    def _sync_bottom_bar(self, index: int) -> None:
        ...
        hints = (
            ...
            "One block per manifest lookup phase — Ctrl+A / Ctrl+C / arrows",
    ...
    def append_save_fetch_per_game(self, d: Dict[str, Any]) -> None:
        ...
        wo = str(d.get("scan_outcome", d.get("wiki_outcome", "?")))[:20]
        line = f"[{ts}] {wall:7.3f}s | {wo:20} | {found:5} | {src:12} | {g}"
