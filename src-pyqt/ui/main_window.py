...
from core.backup_paths import suggested_default_backup_path
from core.catalog_detection_filter import filter_games_for_rescan
from core.custom_game_catalog import (
    add_custom_game as catalog_add_custom_game,
    assign_registry_save_for_row,
    assign_save_folder_for_row,
    row_has_save_location,
)
from core.backup_retention_verifier import (
    get_compress_candidates,
    has_retention_artifact,
    sanitized_game_names_for_filter,
    try_get_open_game_backups_folder_path,
)
...
from core.game_catalog_restore import (
    finalize_scan_game_infos,
    is_scan_derived_game_info,
    mark_saved_game_list_established,
    restore_game_infos_from_catalog,
    scan_result_to_game_info,
...
from ui.custom_dialogs import AddCustomGameDialog, AssignRegistrySaveDialog, SettingsDialog, FirstBackupDestinationDialog
...
from config.settings_defaults import settings_bool, settings_str
from core.window_size import (
    PRESET_CUSTOM,
...
class FadeInItemDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        col = index.column()
        model = index.model()
        total_cols = model.columnCount() if model else 5
...
class GameTableWidget(QTableWidget):
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() == Qt.Key.Key_F10 and event.modifiers() & Qt.KeyboardModifier.ShiftModifier:
            ...
            if idx.isValid():
                pos = self.visualRect(idx).center()
            else:
    ...
    def resizeEvent(self, event):
        """Keep fixed columns at correct widths; Game Name fills remaining space."""
...
class MainWindow(QMainWindow):
    def __init__(self, sandbox_monitor=None):
        ...
        from config.app_config import app_settings, DEFAULT_UI_THEME, normalize_ui_theme
        ...
        refresh_manifest_action = self.scan_menu.addAction("Download latest manifest and rescan")
        refresh_manifest_action.triggered.connect(self.refresh_manifest_and_rescan)
        ...
        self.manifest_provider = LudusaviManifestProvider()
        self.games_data = {}
        ...
        self._footer_min_client_width = self._compute_footer_minimum_client_width()
        self._footer_min_client_height = self._compute_footer_minimum_client_height()
        self._suppress_window_resize_persist = False
        self._window_resize_debounce = QTimer(self)
        self._window_resize_debounce.setSingleShot(True)
        self._window_resize_debounce.setInterval(400)
        self._window_resize_debounce.timeout.connect(self._on_window_resize_debounce)
        self._apply_main_window_layout_from_settings()

    def _compute_footer_minimum_client_width(self) -> int:
        """Footer row width at longest filter label (computed once — no dynamic reflow)."""
        from core.window_size import FALLBACK_MIN_W_RETAIL, FALLBACK_MIN_W_SANDBOX

        saved = self.filter_button.text()
        self.filter_button.setText("Filter: Not Found")
        self.filter_button.adjustSize()
        hint = self.central_widget.minimumSizeHint().width()
        self.filter_button.setText(saved)
        floor = FALLBACK_MIN_W_SANDBOX if self._sandbox_monitor else FALLBACK_MIN_W_RETAIL
        return max(int(hint) + 12, floor)

    def _compute_footer_minimum_client_height(self) -> int:
        hint = self.central_widget.minimumSizeHint().height()
        return max(int(hint), 400)

    def _apply_main_window_resize_policy(self, locked: bool, w: int, h: int) -> None:
        if locked:
            self.setMinimumSize(w, h)
            self.setMaximumSize(w, h)
        else:
            self.setMinimumSize(self._footer_min_client_width, self._footer_min_client_height)
            self.setMaximumSize(16777215, 16777215)

    def _apply_main_window_layout_from_settings(self) -> None:
        preset = settings_str(self.settings, "main_window_client_preset")
        custom_w = int(self.settings.value("main_window_custom_width", 0, type=int))
        custom_h = int(self.settings.value("main_window_custom_height", 0, type=int))
        locked = settings_bool(self.settings, "main_window_lock_resolution")
        w, h = resolve_client_size(
            preset,
            custom_w,
            custom_h,
            min_w=self._footer_min_client_width,
            min_h=self._footer_min_client_height,
        )
        self._suppress_window_resize_persist = True
        try:
            self.resize(w, h)
            self._apply_main_window_resize_policy(locked, w, h)
        finally:
            self._suppress_window_resize_persist = False

    def apply_window_layout_preview(
        self,
        preset: str,
        custom_w: int,
        custom_h: int,
        ...
        if preset == PRESET_CUSTOM:
            custom_w, custom_h = self.width(), self.height()
        w, h = resolve_client_size(
            preset,
            custom_w,
            custom_h,
    ...
    def capture_window_layout_snapshot(self) -> dict:
        return {
            "preset": settings_str(self.settings, "main_window_client_preset"),
            "custom_w": int(self.settings.value("main_window_custom_width", 0, type=int)),
            "custom_h": int(self.settings.value("main_window_custom_height", 0, type=int)),
    ...
    def restore_window_layout_snapshot(self, snapshot: dict) -> None:
        if not snapshot:
            return
        self.settings.setValue("main_window_client_preset", snapshot.get("preset", "800x600"))
        self.settings.setValue("main_window_custom_width", snapshot.get("custom_w", 0))
        self.settings.setValue("main_window_custom_height", snapshot.get("custom_h", 0))
        self.settings.setValue("main_window_lock_resolution", bool(snapshot.get("locked")))
        locked = bool(snapshot.get("locked"))
        w = int(snapshot.get("w", self.width()))
        h = int(snapshot.get("h", self.height()))
        self._suppress_window_resize_persist = True
        try:
            self.resize(w, h)
            self._apply_main_window_resize_policy(locked, w, h)
    ...
    def persist_main_window_client_resize(self, width: int, height: int) -> None:
        w = max(self._footer_min_client_width, int(width))
        h = max(self._footer_min_client_height, int(height))
        self.settings.setValue("main_window_client_preset", PRESET_CUSTOM)
        self.settings.setValue("main_window_custom_width", w)
        self.settings.setValue("main_window_custom_height", h)

    def _on_window_resize_debounce(self) -> None:
        if self._suppress_window_resize_persist:
            return
        if settings_bool(self.settings, "main_window_lock_resolution"):
            return
        self.persist_main_window_client_resize(self.width(), self.height())

    def resizeEvent(self, event):
        super().resizeEvent(event)
        if getattr(self, "_suppress_window_resize_persist", False):
            return
        if settings_bool(self.settings, "main_window_lock_resolution"):
            return
        if hasattr(self, "_window_resize_debounce"):
            self._window_resize_debounce.start()
    ...
    def _set_scan_button_idle_text(self) -> None:
        ...
        # Return visit: restore saved list; first launch stays empty until Scan.
        self.try_restore_saved_game_list_on_startup()
    ...
    def _should_show_notification(self):
        ...
        return (settings_bool(self.settings, "notifications_enabled") and
    ...
    def _show_tray_notification(self, title, message, icon=QSystemTrayIcon.MessageIcon.Information, duration=2000):
        ...
        sound_enabled = settings_bool(self.settings, "notification_sound_enabled")
    ...
    def on_auto_backup_completed(self, game_name, success, message):
        ...
        if success:
            ...
            if not self._footer_operation_busy():
    ...
    def open_settings(self):
        snapshot = self.capture_window_layout_snapshot()
        ...
        if dialog.exec():
            ...
            self.restore_window_layout_snapshot(snapshot)
    ...
    def refresh_backup_dates(self):
        ...
        for row in range(self.game_table_widget.rowCount()):
            game_info = self._row_game_info(row)
            if game_info:
    ...
    def closeEvent(self, event):
        ...
        minimize_to_tray = settings_bool(self.settings, "minimize_to_tray")
        ...
        if minimize_to_tray and hasattr(self, 'tray_icon') and self.tray_icon.isVisible():
            ...
            return
        # Actually exiting
        ask_compress = settings_bool(self.settings, "ask_compress_on_exit")
    ...
    def setup_game_table(self):
        ...
        self.game_table_widget.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)
        self.game_table_widget.customContextMenuRequested.connect(self._on_game_table_context_menu)
        ...
        header.setSectionResizeMode(3, QHeaderView.ResizeMode.Fixed)        # Last Backup - fixed at 200px
    ...
    def _on_section_resized(self, logicalIndex, oldSize, newSize):
        ...
        if logicalIndex == 0:
            ...
            fixed = {1: 140, 2: 140, 3: 200}
            if logicalIndex in fixed:
                self.game_table_widget.setColumnWidth(logicalIndex, fixed[logicalIndex])
    ...
    def _row_game_info(self, row: int):
        item = self.game_table_widget.item(row, 0)
        if not item:
            return None
        return item.data(Qt.ItemDataRole.UserRole)

    def _set_row_game_info(self, row: int, game_info: dict) -> None:
        item = self.game_table_widget.item(row, 0)
        if item:
            item.setData(Qt.ItemDataRole.UserRole, game_info)
    ...
    def _game_has_known_save_location(self, game_info: dict) -> bool:
        return row_has_save_location(game_info or {})
    ...
    def _add_game_to_table_widget(self, game_info, animate=True):
        ...
        if items:
            row = items[0].row()
            old_info = self._row_game_info(row)
    ...
    def try_restore_saved_game_list_on_startup(self):
        """Hydrate the grid from disk on return visits (WinUI TryRestoreSavedGameListOnStartup parity)."""
        from config.sandbox_defaults import ignore_cached_game_list

        should_restore, mark_migration = startup_restore_plan(self.save_manager, self.settings)
        if not should_restore:
            return

        if mark_migration:
            mark_saved_game_list_established(self.settings)
        ...
        dedupe = not self.settings.value("show_duplicate_save_titles", False, type=bool)
        infos, status = restore_game_infos_from_catalog(self.save_manager, dedupe_shared=dedupe)
        if not infos:
            return

        for info in infos:
            self._add_game_to_table_widget(info, animate=False)
        self.status_label.setText(status)
        self._update_backup_compress_buttons()
        self._apply_filter()

    def _existing_game_name_set(self) -> set:
        names: set = set()
        for row in range(self.game_table_widget.rowCount()):
            info = self._row_game_info(row)
            if info and info.get("name"):
                names.add(info["name"])
        return names

    def _clear_scan_derived_table_rows(self):
        """Remove install-scan rows before a new scan (custom / user-added games are kept)."""
        rows_to_remove: list[int] = []
        for row in range(self.game_table_widget.rowCount()):
            info = self._row_game_info(row)
            if info and is_scan_derived_game_info(info, self.save_manager):
                rows_to_remove.append(row)
        for row in reversed(rows_to_remove):
            info = self._row_game_info(row)
            if info:
                self.games_data.pop(info.get("name"), None)
            self.game_table_widget.removeRow(row)

    def _remove_scan_rows_by_name(self, names: list):
        """After same-save-folder dedupe, drop merged-away titles from the grid."""
        if not names:
            return
        drop = {str(n).lower() for n in names}
        for row in range(self.game_table_widget.rowCount() - 1, -1, -1):
            info = self._row_game_info(row)
            if not info or not is_scan_derived_game_info(info, self.save_manager):
                continue
            if str(info.get("name") or "").lower() in drop:
                self.games_data.pop(info.get("name"), None)
                self.game_table_widget.removeRow(row)

    def _finalize_scan_into_game_list(self, detected_games, scanned_for_save_lookup):
        """Re-add skipped installs and custom catalog rows after save lookup finishes."""
        existing = self._existing_game_name_set()
        for info in finalize_scan_game_infos(
            detected_games, scanned_for_save_lookup, self.save_manager, existing
        ):
            self._add_game_to_table_widget(info, animate=False)
    ...
    def _footer_operation_busy(self) -> bool:
        """True while scan, manual backup, or compress holds the footer."""
        if self.is_scanning:
            return True
        if getattr(self, "_compress_running", False):
            return True
        worker = getattr(self, "backup_worker", None)
        if worker is not None and worker.isRunning():
            return True
        return False

    def _update_backup_compress_buttons(self):
        """Enable Backup when a save exists; Compress when any game has backup artifacts."""
        has_backup = False
        has_compress = False
        backup_root = self._effective_default_backup_path() or self._effective_last_backup_path()
        subfolder = self.settings.value("backup_subfolder_per_game", True, type=bool)
        for row in range(self.game_table_widget.rowCount()):
            game_info = self._row_game_info(row)
            if not game_info:
                continue
            if self._can_backup_game_save(game_info):
                has_backup = True
            if backup_root and has_retention_artifact(backup_root, game_info.get("name", ""), subfolder):
                has_compress = True
        self.backup_selected_button.setEnabled(has_backup)
        self.compress_button.setEnabled(has_compress)
        if has_compress:
            sel = sorted(set(i.row() for i in self.game_table_widget.selectedItems()))
            if sel:
                self.compress_button.setToolTip(
                    "Compress backups for selected games that have been backed up at least once."
                )
            else:
                self.compress_button.setToolTip(
                    "Compress backups for all games that have been backed up at least once."
                )
        else:
            self.compress_button.setToolTip(
                "No backed-up games to compress yet. Run a backup first."
            )
    ...
    def start_game_scan(self):
        ...
        self.backup_selected_button.setEnabled(False)
        self.compress_button.setEnabled(False)
        self._clear_scan_derived_table_rows()
        ...
        try:
            self.manifest_provider.load_manifest_offline_only()
        except Exception as exc:
            print(f"Warning: could not load Ludusavi manifest offline: {exc}")
    ...
    def cancel_scan(self):
        ...
        self.is_scanning = False
        self._sandbox_log("scan", "Scan cancel requested (stopping game detection or save lookup).")
    ...
    def refresh_manifest_and_rescan(self):
        if self.is_scanning:
            QMessageBox.information(
                self,
                "Manifest refresh",
                "Please wait for the current scan to finish or cancel it before refreshing the manifest.",
            )
            return

        self.status_label.setText("Downloading latest save-path manifest…")
        self.scan_button_container.setEnabled(False)
        self._sandbox_log("scan", "Ludusavi manifest refresh started (GitHub YAML → local cache).")
        self._manifest_refresh_thread = ManifestRefreshWorker(self.manifest_provider)
        self._manifest_refresh_thread.finished.connect(self._on_manifest_refresh_finished)
        self._manifest_refresh_thread.error.connect(self._on_manifest_refresh_error)
        self._manifest_refresh_thread.start()

    def _on_manifest_refresh_error(self, message: str):
        self.scan_button_container.setEnabled(True)
        self.status_label.setText(f"Manifest refresh failed: {message}")
        self._sandbox_log("scan", f"Manifest refresh error: {message}")
        QMessageBox.warning(self, "Manifest refresh", f"Could not refresh the save-path manifest:\n{message}")

    def _on_manifest_refresh_finished(self, status: str):
        self.scan_button_container.setEnabled(True)
        messages = {
            "updated": "Manifest updated. Starting scan…",
            "not_modified": "Manifest already up to date. Starting scan…",
            "network_error": "Network error while downloading manifest.",
            "yaml_error": "Downloaded manifest could not be parsed.",
            "manifest_too_large": "Downloaded manifest exceeded the size limit.",
            "not_modified_without_cache": "Server reported not modified but no local cache exists.",
        }
        text = messages.get(status, f"Manifest refresh finished ({status}). Starting scan…")
        self.status_label.setText(text)
        self._sandbox_log("scan", f"Manifest refresh finished: {status}")
        if status in ("network_error", "yaml_error", "manifest_too_large", "not_modified_without_cache"):
            QMessageBox.warning(self, "Manifest refresh", text)
            return
        self.start_game_scan()
    ...
    def on_game_detection_finished(self, detected_games):
        ...
        if self._sandbox_monitor:
            if not self.is_scanning:
                ...
                self._sandbox_log("scan", f"Local detection finished in {dt:.2f}s | {n} game(s) found (before save lookup filter).")
        ...
        if not detected_games:
            mark_saved_game_list_established(self.settings)
            self._finalize_scan_into_game_list([], [])
            row_count = self.game_table_widget.rowCount()
            self.status_label.setText(
        ...
        games_to_scan = filter_games_for_rescan(
            detected_games,
            self.save_manager.game_save_locations,
            skip_when_previously_not_found=skip_not_found,
        )
        self._last_detected_games = detected_games
        self._last_games_to_scan = games_to_scan

        if not games_to_scan:
            mark_saved_game_list_established(self.settings)
            self._finalize_scan_into_game_list(detected_games, games_to_scan)
            row_count = self.game_table_widget.rowCount()
            self.status_label.setText(
                "Nothing new to look up — skipped titles are unchanged. Custom games remain listed."
                if row_count > 0
                else (
                    "No titles left to scan. Games previously remembered with no usable save folder "
                    "are skipped — remove those rows from the list or edit game_save_data.json if you "
        ...
        self._sandbox_manifest_t0 = time.perf_counter()
        self._sandbox_log(
            "scan",
            f"Ludusavi manifest save lookup started for {len(games_to_scan)} game(s) (offline + local cache writes).",
        )
        dedupe_shared_saves = not self.settings.value("show_duplicate_save_titles", False, type=bool)
        self.save_fetcher_thread = SaveLocationFetcherWorker(
            games_to_scan,
            self.steam_ids,
            self.manifest_provider,
            self.save_manager,
            deduplicate_shared_save_folders=dedupe_shared_saves,
        )
        self.save_fetcher_thread.game_save_fetched.connect(self.on_save_location_fetched)
        self.save_fetcher_thread.save_fetch_progress.connect(self._on_save_fetch_progress)
        self.save_fetcher_thread.all_fetching_finished.connect(self.on_all_save_locations_fetched)
        self.save_fetcher_thread.games_dropped_from_dedup.connect(self._remove_scan_rows_by_name)
    ...
    def _on_save_fetch_progress(self, completed: int):
        self.progress_bar.setValue(completed)
        total = getattr(self, "detected_games_count", 0)
        if total:
            self.status_label.setText(f"Looking up save paths… ({completed}/{total})")
    ...
    def on_save_location_fetched(self, game_data):
        ...
        if self._sandbox_monitor and self.detected_games_count > 0:
            ...
            if self.processed_games_count % step == 0 or self.processed_games_count == self.detected_games_count:
                wdt = time.perf_counter() - getattr(self, "_sandbox_manifest_t0", time.perf_counter())
                g = game_data.get("name", "?")
                self._sandbox_log(
                    "scan",
                    f"… {self.processed_games_count} kept ({wdt:.1f}s elapsed) last: {g!r}",
                )
        self.status_label.setText(f"Applying results… ({self.processed_games_count} game(s))")
        self._add_game_to_table_widget(scan_result_to_game_info(game_data))
    ...
    def on_all_save_locations_fetched(self):
        if self._sandbox_monitor and getattr(self, "_sandbox_manifest_t0", None):
            wdt = time.perf_counter() - self._sandbox_manifest_t0
            avg = wdt / max(1, self.processed_games_count)
            self._sandbox_log(
                "scan",
                f"Manifest lookup done in {wdt:.2f}s | {self.processed_games_count} game(s) kept (~{avg:.2f}s each avg).",
            )
            self._sandbox_monitor.append_save_fetch_batch_footer(self.processed_games_count, wdt)
        mark_saved_game_list_established(self.settings)
        self._finalize_scan_into_game_list(
            getattr(self, "_last_detected_games", []),
            getattr(self, "_last_games_to_scan", []),
        )
        self.is_scanning = False
        self.scan_button_container.setEnabled(True)
        self.scan_arrow_button.setVisible(True)
        ...
        row_count = self.game_table_widget.rowCount()
        self.status_label.setText(f"Scan complete. {row_count} game(s) in catalog.")
        if self._should_show_notification():
            self._show_tray_notification("Scan complete", f"{row_count} game(s) in catalog.", QSystemTrayIcon.MessageIcon.Information, 2000)
        self._apply_filter()
        
        # Start file monitoring with updated game data
        self.start_file_monitoring()

    def _trusted_large_save_paths(self) -> list:
        v = self.settings.value("trusted_large_save_paths", [])
        return list(v) if isinstance(v, list) else []

    def _trust_large_save_path(self, game_name: str) -> None:
        paths = self._trusted_large_save_paths()
        if not any(p.lower() == game_name.lower() for p in paths):
            paths.append(game_name)
            self.settings.setValue("trusted_large_save_paths", paths)

    def _untrust_large_save_path(self, game_name: str) -> None:
        paths = [p for p in self._trusted_large_save_paths() if p.lower() != game_name.lower()]
        self.settings.setValue("trusted_large_save_paths", paths)

    def _open_registry_save_info(self, game_info: dict) -> None:
    ...
    def _open_save_folder_for_game(self, game_info: dict) -> None:
        if game_info.get("save_in_registry_only"):
            self._open_registry_save_info(game_info)
            return
        path = game_info.get("save_path_resolved")
        if path and os.path.isdir(str(path)):
            try:
                os.startfile(str(path))
            except Exception as exc:
                QMessageBox.critical(self, "Error", f"Could not open folder: {exc}")
        else:
            QMessageBox.warning(self, "Path Not Found", f"The save path for '{game_info.get('name')}' could not be found.")

    def _open_backup_folder_for_game(self, game_info: dict) -> None:
        backup_root = self._effective_default_backup_path() or self._effective_last_backup_path()
        if not backup_root:
            QMessageBox.warning(self, "No backup folder", "Set a default backup folder in Settings.")
            return
        subfolder = self.settings.value("backup_subfolder_per_game", True, type=bool)
        ok, path, hint = try_get_open_game_backups_folder_path(
            backup_root, game_info.get("name", ""), subfolder
        )
        if not ok or not path:
            QMessageBox.warning(self, "Backup folder", hint or "Could not open backup folder.")
            return
        try:
            os.startfile(path)
        except Exception as exc:
    ...
    def _update_row_from_game_info(self, row: int, game_info: dict) -> None:
        name_item = self.game_table_widget.item(row, 0)
        if name_item:
            name_item.setData(Qt.ItemDataRole.UserRole, game_info)
        status = "✓ Found" if self._game_has_known_save_location(game_info) else "✗ Not Found"
        st_item = self.game_table_widget.item(row, 2)
        if st_item:
            st_item.setText(status)
        self.games_data[game_info.get("name", "")] = game_info

    def _remove_game_row_by_name(self, game_name: str) -> None:
        items = self.game_table_widget.findItems(game_name, Qt.MatchFlag.MatchExactly)
        if not items:
            return
        row = items[0].row()
        ...
        self._last_deleted_entries = [{"name": game_name, "game_info": game_info, "save_data": save_data}]
        self.game_table_widget.removeRow(row)
        self.save_manager.delete_games([game_name])
        self._untrust_large_save_path(game_name)

    def _on_game_table_context_menu(self, pos) -> None:
        index = self.game_table_widget.indexAt(pos)
        ...
        if not game_info:
            return
        menu = QMenu(self)
        menu.setStyleSheet(self._styles.menu_qss())

        open_save = menu.addAction("Open save path")
        open_save.setEnabled(
            bool(game_info.get("save_in_registry_only"))
            or bool(
                game_info.get("save_path_resolved")
                and os.path.isdir(str(game_info.get("save_path_resolved")))
            )
        )
        open_save.triggered.connect(lambda: self._open_save_folder_for_game(game_info))

        open_backup = menu.addAction("Open backup path")
        open_backup.setEnabled(bool(self._effective_default_backup_path() or self._effective_last_backup_path()))
        open_backup.triggered.connect(lambda: self._open_backup_folder_for_game(game_info))

        if not self._game_has_known_save_location(game_info):
            add_folder = menu.addAction("Add save folder…")
            add_folder.triggered.connect(lambda: self._assign_save_folder_dialog_for_row(row, game_info))
            add_reg = menu.addAction("Add registry path…")
            add_reg.triggered.connect(lambda: self._assign_registry_dialog_for_row(row, game_info))

        menu.exec(self.game_table_widget.viewport().mapToGlobal(pos))

    def _assign_save_folder_dialog_for_row(self, row: int, game_info: dict) -> None:
        folder = QFileDialog.getExistingDirectory(self, "Select save folder")
        if not folder:
            return
        ok, msg, updated = assign_save_folder_for_row(
            self.save_manager, game_info.get("name", ""), folder, game_info
        )
        if not ok:
            QMessageBox.warning(self, "Save folder", msg)
            return
        self._update_row_from_game_info(row, updated)
        self._update_backup_compress_buttons()
        self.status_label.setText(msg)
        self.start_file_monitoring()

    def _assign_registry_dialog_for_row(self, row: int, game_info: dict) -> None:
        dlg = AssignRegistrySaveDialog(game_info.get("name", ""), self)
        if dlg.exec() != QDialog.DialogCode.Accepted:
            return
        ok, msg, updated = assign_registry_save_for_row(
            self.save_manager, game_info.get("name", ""), dlg.get_registry_path(), game_info
        )
        if not ok:
            QMessageBox.warning(self, "Registry path", msg)
            return
        self._update_row_from_game_info(row, updated)
        self._update_backup_compress_buttons()
        self.status_label.setText(msg)
    ...
    def on_game_double_clicked(self, item):
        ...
        if resolved_save_path and os.path.exists(resolved_save_path):
            try:
                os.startfile(resolved_save_path)
            except Exception as e:
    ...
    def add_custom_game(self):
        ...
        if not name:
            QMessageBox.warning(self, "Missing Information", "Game name is required.")
            return
        if data.get("mode") == AddCustomGameDialog.MODE_REGISTRY:
            ok, msg, game_info = catalog_add_custom_game(
                self.save_manager, name, registry_path=data.get("registry_path")
            )
        else:
            ok, msg, game_info = catalog_add_custom_game(
                self.save_manager, name, folder_path=data.get("save_path")
            )
        if not ok:
            QMessageBox.warning(self, "Add custom game", msg)
            return
        self._add_game_to_table_widget(game_info)
        self._update_backup_compress_buttons()
        self.status_label.setText(msg)
    ...
    def backup_selected_saves(self):
        ...
        if default_backup_path and os.path.exists(default_backup_path):
            ...
            fallback = self._effective_last_backup_path() or suggested_default_backup_path()
        ...
        self._run_backup_estimate_async(games_to_backup, destination_folder, subfolder_per_game, want_est)
    ...
    def _prompt_backup_start(self, games_to_backup, destination_folder, est, *, want_est: bool):
        """Show backup estimate dialog when enabled or when large-folder warnings exist."""
        if est is None:
            return True, games_to_backup
        summary = est.get("summary")
        if not summary:
            return True, games_to_backup
        must_show = want_est or summary.has_severity_warnings
        if not must_show:
            return True, games_to_backup
        warning_only = (not want_est) and summary.has_severity_warnings
        trusted = set(self._trusted_large_save_paths())
        dlg = BackupEstimatePromptDialog(
            self,
            est,
            len(games_to_backup),
            destination_folder,
            want_confirm=want_est,
            warning_only=warning_only,
            trusted_names=trusted,
            on_trust_path=self._trust_large_save_path,
            on_remove_game=self._remove_game_row_by_name,
        )
        if dlg.exec() != QDialog.DialogCode.Accepted:
            return False, games_to_backup
        removed = {n.lower() for n in dlg.removed_game_names}
        if removed:
            games_to_backup = [
                g for g in games_to_backup if g.get("name", "").lower() not in removed
            ]
        return True, games_to_backup

    def _run_backup_estimate_async(self, games_to_backup, destination_folder, subfolder_per_game, want_est):
        ...
        worker = BackupEstimateWorker(
            games_to_backup,
            destination_folder,
            trusted_names=self._trusted_large_save_paths(),
            parent=self,
        )
        ...
        def on_ok(est):
            prog.close()
            cleanup_worker()
            proceed, filtered = self._prompt_backup_start(
                games_to_backup, destination_folder, est, want_est=want_est
            )
            if not proceed:
                return
            if not filtered:
                QMessageBox.information(self, "Backup", "No games left to back up.")
                return
            self._start_backup_worker(filtered, destination_folder, subfolder_per_game)
    ...
    def compress_backups(self, then_exit=False, from_tray=False):
        ...
        subfolder_per_game = self.settings.value("backup_subfolder_per_game", True, type=bool)
        all_names = [
            self._row_game_info(row).get("name", "")
            for row in range(self.game_table_widget.rowCount())
            if self._row_game_info(row)
        ]
        selected_rows = sorted(set(item.row() for item in self.game_table_widget.selectedItems()))
        selected_names = [
            self._row_game_info(row).get("name", "")
            for row in selected_rows
            if self._row_game_info(row)
        ]
        candidates = get_compress_candidates(
            all_names, backup_path, subfolder_per_game, selected_names or None
        )
        if not candidates:
            if selected_names:
                QMessageBox.information(
                    self,
                    "Nothing to compress",
                    "None of the selected games have backup files yet. Back them up first, or clear the selection to compress all backed-up games.",
                )
            else:
                QMessageBox.information(
                    self,
                    "Nothing to compress",
                    "No games have backup files yet. Run a backup first.",
                )
            return
        sanitized = sanitized_game_names_for_filter(candidates)
        ...
        self.compress_worker = CompressBackupWorker(
            ...
            sanitized_game_names=sanitized,
        )
