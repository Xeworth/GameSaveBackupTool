...
def strip_one_trailing_parenthetical(title: str) -> Optional[str]:
    """
    Strip a single trailing '(…)' disambiguator. Steam often uses 'Game (Classic)' while
    PCGW uses the base page title 'Game' (e.g. Mafia II (Classic) → Mafia II).
    """
    if not title or not isinstance(title, str):
        return None
    m = re.match(r"^(?P<base>.+)\s\([^)]+\)\s*$", title.strip())
    if not m:
        return None
...
def wiki_title_resolution_order(primary: str) -> List[str]:
    """
    Wiki titles to try in order: exact name, then repeatedly strip trailing '(Edition)'.
    ...
    while cur:
        if cur not in seen:
            seen.add(cur)
            titles.append(cur)
        nxt = strip_one_trailing_parenthetical(cur)
...
def normalize_wiki_section_heading_line(line: str) -> str:
    """Normalize a MediaWiki section TOC line for comparison (strip punctuation, collapse spaces)."""
    if not line:
        return ""
...
def normalized_section_is_save_location_heading(norm: str) -> bool:
    """
    True if a MediaWiki TOC line (already normalized like _normalize_section_line) is the
    PCGW save-location section, excluding cloud sync / config-only headings.
    """
    if not norm:
        return False
    if "configuration" in norm:
        return False
    if "cloud" in norm and "sync" in norm:
        return False
    if norm == "save game data location":
        return True
    if norm in ("save game location", "save location", "game save location"):
        return True
    if "save" in norm and "game" in norm and "data" in norm and "location" in norm:
        return True
    return False


def wiki_section_heading_is_save_game_location(line: Optional[str]) -> bool:
    """True if this section heading is the save-game location block (not cloud sync / config)."""
    return normalized_section_is_save_location_heading(normalize_wiki_section_heading_line(line or ""))
...
class SaveLocationFetcher:
    ...
    def _fallback_parse_html_section(self, page_title):
        ...
        try:
            ...
            sec_json = response_sections.json()
            if sec_json.get("error"):
                code = sec_json["error"].get("code", "?")
                print(f"--> Wiki parse (sections) failed for '{page_title}': {code}")
                return None
            sections = sec_json.get("parse", {}).get("sections", []) or []
            save_section_index = None
            for s in sections:
                line = s.get("line", "")
                if normalize_wiki_section_heading_line(line) == "save game data location":
                    save_section_index = s.get("index")
                    break
            if not save_section_index:
                for s in sections:
                    if wiki_section_heading_is_save_game_location(s.get("line", "")):
                        save_section_index = s.get("index")
                        break
            if not save_section_index:
                return None

            print(f"--> Found save location section as index {save_section_index}. Fetching its HTML...")
            self._rate_limit()
            params_html = {
                "action": "parse",
                "page": page_title,
    ...
    def fetch_save_location(self, game_name, steam_app_id):
        ...
        for title in wiki_title_resolution_order(clean_name):
            tried_titles.add(title)
            print(f"--> Fetching semantic data from wiki page: '{title}'...")
            result = self._fetch_for_page_title(title)
            if result:
                return result
        # No paths: search and pick closest match (e.g. "Lego Star Wars: The Complete Saga")
        page_title = self._get_page_name_from_search(clean_name)
        if page_title and page_title not in tried_titles:
            print(f"--> Fetching semantic data from wiki page: '{page_title}'...")
            tried_titles.add(page_title)
        ...
        for variant in build_title_variants_for_fallback(clean_name):
            if variant == clean_name:
                continue
            for title in wiki_title_resolution_order(variant):
                if title in tried_titles:
                    continue
                tried_titles.add(title)
                print(f"--> Trying wiki title variant: '{title}'...")
                result = self._fetch_for_page_title(title)
                if result:
                    return result
        return None