...
# Optional user overrides via config/steam_registry_save_keys.json (no bundled game entries).
...
def merged_steam_registry_save_keys() -> Dict[str, Tuple[str, str]]:
    ...
    for cfg in (
        Path(__file__).resolve().parent.parent / "config" / "steam_registry_save_keys.json",
        Path(user_data_file("steam_registry_save_keys.json")),
    ):
        if not cfg.is_file():
            continue
        try:
