...
class GameDetector:
    ...
    def detect_steam_games(self):
        ...
        return self._dedupe_steam_shared_install_folder(found_games)

    @staticmethod
    def _dedupe_steam_shared_install_folder(games):
        """Merge rows that share one install directory (DLC app IDs); keep lowest numeric app id."""
        groups: dict[str, list] = {}
        for game in games:
            install_path = game.get("install_path") or ""
            try:
                key = os.path.normpath(install_path).lower()
        ...
        for group in groups.values():
            ...
            for row in group:
                ...
                if numeric_id < best_id or (
                    numeric_id == best_id
                    and str(row.get("name") or "") < str(best.get("name") or "")
                ):
                    best = row
                    best_id = numeric_id
            if best is not None:
