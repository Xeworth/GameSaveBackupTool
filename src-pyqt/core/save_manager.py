...
def default_game_save_data_path() -> str:
    return user_data_file("game_save_data.json")


def _dev_legacy_catalog_path() -> str:
    return str(Path(__file__).resolve().parent.parent / "config" / "game_save_data.json")


class SaveManager:
    def __init__(self, game_save_data_path: str | None = None):
        self.game_save_data_path = game_save_data_path or default_game_save_data_path()
    ...
    def _load_game_save_data(self):
        ...
        """
        candidates = [self.game_save_data_path, _dev_legacy_catalog_path()]
        for path in candidates:
            if os.path.exists(path):
                try:
                    with open(path, "r", encoding="utf-8") as f:
                        data = json.load(f)
                    if path != self.game_save_data_path:
                        self._save_game_save_data_to(data)
                    return data
                except json.JSONDecodeError as e:
                    print(f"Error decoding {path}: {e}. Returning empty data.")
                    return {}
        return {}

    def _save_game_save_data_to(self, data: dict) -> None:
    ...
    def _save_game_save_data(self):
        ...
        """
        self._save_game_save_data_to(self.game_save_locations)
